...
from src.utils.eval_utils import get_anomalies, anomaly_scoring, get_eval_metrics
from src.utils.preprocessing_utils import create_sequences, create_dataframe_of_predicted_labels, \
    transform_df
...
class Inferrer(BaseInferrer):
    ...
    def predict(self):
        ...
        anomalous_data_indices = get_anomalies(self.model,
...
if __name__ == '__main__':
    ...
    for msg in input_fn:
        ...
        if len(queue) >= 11:
            ...
            infer.load_data_online(seq_df)
            pred_df = infer.predict()
            pred_list = pred_df.to_dict(orient='records')

            # print('prediction df')
            # print(pred_df)
            # print('output message: ' + str(count - 6 + 1))
