...
def main_transformer(df):
    ...
    t = [('num', MinMaxScaler(feature_range=(0, 1)), numeric_features),
         ('cat', OneHotEncoder(handle_unknown="ignore"), categorical_features)]
    column_transformer = ColumnTransformer(transformers=t, n_jobs=-1, remainder='passthrough')
    transformed_to_df = FunctionTransformer(transformed_data_to_df, validate=False)
    # Create the pipeline
    pipe = Pipeline(memory=None, steps=[
        ('col_transformer', column_transformer),
        ('transformed_to_df', transformed_to_df)], verbose=True)

    pipe.fit(df)
    pipe.total_features = pipe.named_steps['col_transformer'].transformers_[0][1].get_feature_names_out().tolist() + \
                          pipe.named_steps['col_transformer'].transformers_[1][1].get_feature_names_out(
